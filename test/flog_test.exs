defmodule FlogTest do
  ...
  # FIXME:
  @vlan_trunk_port_sniffer "veth1"
  # FIXME:
  @access_port1_sniffer "veth2"
  # FIXME:
  @access_port2_sniffer "veth5"
  ...
  describe "switch:merged_handler:" <>
    ...
    test "Install Flow", state do
      for port <- [state.vlan_trunk, state.port] do
        ...
        options = [cookie: 0x4000000000000000, table_id: 0, priority: 201, match: match]
        :ok = GenServer.cast(Flay, {:flow_install, options, self()})
        :timer.sleep(1000)
        refute_receive %Openflow.ErrorMsg{}, 1000
      end
  ...
  describe "switch:uplink_escalation_flow:" <>
    ...
    test "Install Flow", state do
      ...
      options = [cookie: 0x1000000000000000, table_id: 0, priority: 11, match: match]
      :ok = GenServer.cast(Flay, {:flow_install, options, self()})
      :timer.sleep(1000)
      refute_receive %Openflow.ErrorMsg{}, 1000
    end
  ...
  describe "associate:register_bootstrap_rule:" <>
             "send_flow_rem," <> "dl_src={mac},in_port={vlan_trunk_port}," <> "actions=drop" do
    test "Install Flow", state do
      match =
        Openflow.Match.new(
          in_port: state.vlan_trunk.number,
  ...
  describe "associate:register_bootstrap_rule:" <>
             "send_flow_rem," <> "dl_src={trusted}," <> "actions=drop" do
    test "Install Flow", state do
      for trusted <- @trusted_macs do
  ...
  describe "associate:register_usernet_rule:" <>
             "in_port={vlan_trunk_port},dl_vlan={vlan}," <> "actions=goto_table:1" do
  ...
  describe "associate:register_usernet_rule:" <>
    ...
    test "Install Flow", state do
      ...
      actions = [Openflow.Action.PopVlan.new(), Openflow.Action.Output.new(state.port.number)]
      ins = [Openflow.Instruction.ApplyActions.new(actions)]
  ...
  describe "associate:register_usernet_rule:" <>
             "dl_vlan={vlan}," <> "actions=output:{vlan_trunk_port},strip_vlan,{outputs}" do
    test "Install Flow", state do
  ...
  describe "switch:register_farm_nat_rule:" <>
    ...
    test "Inject Packet(TCP:80)" do
      ...
      expect = [
        ether_header,
        vlan_header,
        ipv4(ipv4_header, sum: 1544),
        tcp(tcp_header, sum: tcp_sum)
      ]
